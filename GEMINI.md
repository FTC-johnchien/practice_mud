...
* **開發原則**：
  ...
  7. **核心架構鐵律：機制運行 + 資料驅動 + 純粹測試 (Mechanism, Data-Driven & Pure Testing Pillars)**：
   ...
   8. **網頁全功能視覺化與統一操作原則 (Web-First Unified UI & Action Rule)**:
...
* [x] **城鎮生靈右側直排佈局、明確功能能力標籤、單人/小隊動態稱謂與角色狀態全覽 (Town 2-Column Body, Explicit Capability Chips, Solo/Party Distinction & Full Character Stats)**：
  ...
  * **Phase 11.5: 戰鬥雙層前後排置中陣線、SP 戰氣動能全面收斂、AOE 傷害日誌顯化與小隊換位系統 [100% 完成]**：
    - **AOE 技能傷害戰鬥日誌顯化**：修復 `DrpgCombatLoop` 中 AOE 招式（如【太陰萬劍訣】）僅有出招宣告而遺漏實質扣血數值日誌之缺陷，現逐隻敵怪完整輸出 `↳ 擊中【敵方】造成 XX 點傷害！`。
    - **SP (戰氣勢能 0~100) 全面收斂**：徹底拔除殘留之「連擊點/怒氣」等分散名詞，資料層與 DTO 全面將物理動態資源收斂為 SP；HUD 與法術書統一展現 HP/MP/SP 三色能量條；受擊/平攻/閃避格擋動態漲氣。
    - **戰鬥主舞台水平置中 + 雙層前後排戰線 (Two-Tier Battle Lines)**：
      - 敵方陣容拆分為「敵方後衛陣線」與「敵方前鋒陣線」，居中交鋒。
      - 我方陣容拆分為「我方前衛陣線」與「我方後衛陣線」，緊扣交鋒線。
      - 徹底解決固定 5 欄網格導致未滿員時卡片全數靠左擠壓之視覺問題。
    - **小隊自由換位 (`party swap <idx1> <idx2>`) 與陣法孔位聯動**：
      - 後端實裝 `Party.swapMembers` 與 `party swap` 指令（支援別名 `換位`、`調位`），隊長身份始終由 `isLeader` 錨定。
      - 隊伍與戰場卡片支援直覺 `◀` / `▶` 換位按鈕；即時動態呈現隊員所繼承之陣法孔位（如 `💠 天璇【玄武巨盾】`），站位不符時即時紅字警示。
    - **機制驗證測試**：新增 `PartySwapAndAoeDamageTest`（3 項全通過），全專案 182 個測試 100% 全綠通過。
...
* **測試項目**：涵蓋既有 179 項測試，以及 Phase 11.5 小隊換位、AOE 傷害日誌與陣法孔位檢定套件（`PartySwapAndAoeDamageTest` 共 3 項新測試）。
* **結果**：`Tests run: 182, Failures: 0, Errors: 0, Skipped: 0` -> **BUILD SUCCESS (182 項測試全數綠燈通過，0 失敗、0 錯誤)**
