...
5. **核心架構鐵律：機制運行 + 資料驅動 + 純粹測試**：嚴禁在 Java 業務代碼中寫死職業、技能、物品或名稱特例（禁止 hardcode `switch-case` / 特例 `if`），所有功能必須由通用機制（Mechanism）支撐，資料完全交由 `data/**/*.json` 驅動（Data-Driven），測試必須驗證機制的正確性與防禦邊界，杜絕死測固定字串。
